...
exports.getAllQuizzes = async (req, res) => {
  try {
    ...
    const quizzes = await Quiz.find({ isActive: true }).select(
      "title description createdAt scheduledStartTime lateJoinWindowMinutes"
    );
...
exports.getQuizById = async (req, res) => {
  try {
    const quiz = await Quiz.findById(req.params.id).select(
      "title description isActive scheduledStartTime lateJoinWindowMinutes scheduledEndTime resultsDeclarationTime"
    );
...
exports.startQuizAttempt = async (req, res) => {
  try {
    ...
    if (quiz.scheduledStartTime) {
      ...
      if (now > lateJoinDeadline) {
        return res.status(403).json({
          success: false,
          message:
            "This quiz is no longer available to join. The scheduled start time plus late join window has passed.",
        });
      }
    }
    ...
    resultsTime.setHours(
      resultsTime.getHours() + (quiz.resultsAvailableAfterHours || 2)
    );
  ...
  try {
    ...
    if (attemptId) {
      try {
        ...
        if (quizAttempt) {
          console.log(
            `Updating correctness for attempt ${attemptId}, question ${questionId}`
          );
          ...
          if (answerIndex !== -1) {
            ...
            console.log(
              `User selected: ${answer.selectedOption}, correct: ${correctOption._id}, isCorrect: ${isCorrect}`
            );
            ...
            if (isCorrect && !quizAttempt.answers[answerIndex].alreadyCounted) {
              ...
              if (scoringType === "simple") {
                ...
                console.log(
                  `Using simple scoring: ${score} point for correct answer`
                );
              } else {
                // Time-based scoring (original method)
                score = calculateTimeBasedScore(
                  answer.timeSpent,
                  question.timeLimit
                );
                console.log(
                  `Using time-based scoring: ${score} points for correct answer in ${answer.timeSpent}/${question.timeLimit} seconds`
                );
              }
            ...
            console.warn(
              `Answer for question ${questionId} not found in attempt ${attemptId}`
            );
          }
        }
      } catch (updateError) {
  ...
  try {
    ...
    const scores = attempts.map((attempt) => {
      return {
        ...
        isCurrentUser:
          attempt._id.toString() === (currentAttemptId || "").toString(),
      };
    });
...
exports.getQuizResults = async (req, res) => {
  try {
    ...
    const quizAttempt = await QuizAttempt.findById(attemptId).populate(
      "quizId"
    );
    if (!quizAttempt) {
      return res
    ...
    res.status(200).json({
      ...
      data: {
        ...
        percentage:
          (quizAttempt.score / (quizAttempt.totalQuestions * 1000)) * 100, // Adjusted for time-based scoring
        rank: userRank,
