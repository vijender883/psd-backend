...
function scheduleSubmissionVisibility(simulationId) {
  ...
  const endTime = new Date(
    startTime.getTime() + (config.testDuration + 10) * 1000
  ...
  if (endTime > now) {
    ...
    console.log(
      `Scheduling visibility update for simulation ${simulationId} in ${
        timeUntilEnd / 1000
      } seconds`
    );
    ...
    setTimeout(async () => {
      try {
        ...
        await Submission.updateMany({ show: false }, { show: true });

        console.log(
          `Automatically updated submission visibility for simulation ${simulationId}`
        );
      } catch (error) {
        console.error(
          `Error updating submission visibility for simulation ${simulationId}:`,
          error
        );
      }
    }, timeUntilEnd);
    ...
    (async () => {
      try {
        ...
        console.log(
          `Immediately updated submission visibility for simulation ${simulationId} (past end time)`
        );
      } catch (error) {
        console.error(
          `Error updating submission visibility for simulation ${simulationId}:`,
...
router.get("/submissions", async (req, res) => {
  try {
    ...
    const submissions = await Submission.find(query)
      ...
      .select(
        "username problemId score passedTests totalTests executionTime createdAt show language"
      );
...
router.post("/analyze", async (req, res) => {
  ...
  console.log(
    `Received submission request from ${username} (ID: ${userId}) for problem ${problemId}`
  );
  ...
  try {
    ...
    if (existingSubmission) {
      ...
      console.log(
        `User ${username} (ID: ${userId}) has already submitted for problem ${problemId}, submission ID: ${existingSubmission._id}`
      );
      return res.json({
        success: true,
    ...
    console.log(
      `Creating new submission record for ${username} (ID: ${userId}) on problem ${problemId}`
    );
    ...
    const submission = new Submission({
      ...
      code,
      pseudocode:
        pseudocode !== undefined && pseudocode !== null ? pseudocode : "",
      timeComplexity:
        timeComplexity !== undefined && timeComplexity !== null
          ? timeComplexity
          : existingSubmission.timeComplexity || "",
      spaceComplexity:
        spaceComplexity !== undefined && spaceComplexity !== null
          ? spaceComplexity
          : existingSubmission.spaceComplexity || "",
      language,
      executionTime: 0,
      score: 0,
      passedTests: 0,
    ...
    console.log(
      `Initiating background processing for submission ${submission._id}`
    );
    processSubmissionAsync(submission._id, code, problem, language).catch(
      (err) =>
        console.error(
          `Background processing error for submission ${submission._id}:`,
          err
        )
    );
    ...
    console.log(
      `Returning success response to client for submission ${submission._id}`
    );
    res.json({
      success: true,
    ...
    if (error.code === 11000) {
      console.log(
        `Duplicate submission detected from ${username} for problem ${problemId}`
      );
...
router.get("/debug/submission/:id", async (req, res) => {
  try {
    ...
    console.log(
      `Processing status: ${
        submission.processingComplete ? "Complete" : "In Progress"
      }`
    );
...
async function processSubmissionAsync(submissionId, code, problem, language) {
  try {
    console.log(
      `Starting background processing for submission ${submissionId}`
    );
    ...
    console.log(
      `Code execution completed with ${
        result.results ? result.results.filter((r) => r.passed).length : 0
      }/${result.results ? result.results.length : 0} tests passed`
    );
    ...
    const averageExecutionTime =
      totalTests > 0
        ? result.results.reduce((sum, r) => sum + r.executionTime, 0) /
          totalTests
        : 0;
    ...
    console.log(
      `Background processing completed for submission ${submissionId}`
    );
  } catch (error) {
    console.error(
      `Background processing failed for submission ${submissionId}:`,
      error
    );
    ...
    try {
      ...
      console.error(
        `Failed to update submission ${submissionId} after processing error:`,
        updateError
      );
    }
  }
...
router.get(
  "/simulation/:simulationId/problems/:problemId",
  async (req, res) => {
    try {
...
router.post(
  "/simulations/:simulationId/:userId/:username/run",
  async (req, res) => {
    ...
    try {
      ...
      if (existingSubmission) {
        ...
        existingSubmission = await Submission.findByIdAndUpdate(
          existingSubmission._id,
          {
            code: code,
            pseudocode:
              pseudocode !== undefined && pseudocode !== null
                ? pseudocode
                : existingSubmission.pseudocode || "",
            timeComplexity:
              timeComplexity !== undefined && timeComplexity !== null
                ? timeComplexity
                : existingSubmission.timeComplexity || "",
        ...
        console.log(
          `Updated existing submission ${existingSubmission._id} for user ${userId}`
        );
      } else {
        ...
        console.log(
          `Created new draft submission ${existingSubmission._id} for user ${userId}`
        );
      }
...
async function processSimulationSubmissionAsync(
  submissionId,
  code,
  problem,
  language,
  ...
  try {
    console.log(
      `Starting background processing for simulation submission ${submissionId}`
    );
    console.log(`Language: ${language}`);

    // Log problem details
    console.log(
      `Processing simulation problem: ${problem.id} - ${problem.title}`
    );
    ...
    try {
      ...
      const geminiResponse = await axios.post(
        "http://localhost:8002/geminireview",
        {
          userId: userId,
      ...
      if (geminiResponse.data && geminiResponse.data.review) {
        ...
        console.log(
          `✅ Successfully received Gemini review for submission ${submissionId}`
        );
      }
    } catch (geminiError) {
      console.error(
        `❌ Failed to get Gemini review for submission ${submissionId}:`,
        geminiError.message
      );
    ...
    // Update the submission with results
    console.log(
      `Updating simulation submission ${submissionId} with execution results`
    );
    await Submission.findByIdAndUpdate(submissionId, {
      executionTime: averageExecutionTime,
      score,
    ...
    try {
      ...
      await Leaderboard.add_to_leaderboard(
        simulationId,
        userId,
        username,
        score,
      ...
      console.log(
        `✅ Successfully updated leaderboard for user ${userId} (${username}) in simulation ${simulationId} with score ${score}`
      );
    } catch (leaderboardError) {
      console.error(
        `❌ Failed to update leaderboard for user ${userId} (${username}) in simulation ${simulationId}:`,
        leaderboardError
      );
    ...
    console.log(
      `Background processing completed for simulation submission ${submissionId}`
    );
  } catch (error) {
    console.error(
      `Background processing failed for simulation submission ${submissionId}:`,
      error
    );
    ...
    try {
      ...
      console.log(
        `Updated simulation submission ${submissionId} with error information`
      );
    } catch (updateError) {
      console.error(
        `Failed to update simulation submission ${submissionId} after processing error:`,
        updateError
      );
    }
  }
...
router.post("/simulations/:simulationId/analyze", async (req, res) => {
  ...
  console.log(
    `Received submission request from ${username} (ID: ${userId}) for problem ${problemId} in simulation ${simulationId}`
  );
  ...
  try {
    ...
    if (!problem) {
      console.log(
        `Problem ${problemId} not found in simulation ${simulationId}`
      );
      return res.status(404).json({
        success: false,
        error: "Problem not found in simulation",
    ...
    if (existingSubmission) {
      ...
      if (existingSubmission.isSubmitted) {
        console.log(
          `User ${username} (ID: ${userId}) has already submitted for problem ${problemId} in simulation ${simulationId}, submission ID: ${existingSubmission._id}`
        );
        return res.json({
          success: true,
          message: "You have already submitted a solution for this problem",
        ...
        console.log(
          `Updated existing draft submission ${existingSubmission._id} and marked as submitted for user ${userId}`
        );

        // Start processing in the background without waiting for it to complete
        console.log(
          `Initiating background processing for submission ${existingSubmission._id} in simulation ${simulationId}`
        );
        processSimulationSubmissionAsync(
          existingSubmission._id,
          code,
          problem,
          language,
          ...
          console.error(
            `Background processing error for submission ${existingSubmission._id} in simulation ${simulationId}:`,
            err
          )
        );
    ...
    console.log(
      `Creating new submission record for ${username} (ID: ${userId}) on problem ${problemId} in simulation ${simulationId}`
    );
    ...
    await existingSubmission.save();
    console.log(
      `Submission record created with ID: ${existingSubmission._id} for simulation ${simulationId}`
    );
    ...
    // Return success immediately
    console.log(
      `Returning success response to client for submission ${existingSubmission._id} in simulation ${simulationId}`
    );
    res.json({
      success: true,
      message: "Submission received successfully",
    ...
    if (error.code === 11000) {
      console.log(
        `Duplicate submission detected from ${username} for problem ${problemId} in simulation ${simulationId}`
      );
      try {
        const existingSubmission = await Submission.findOne({
          username: username.trim(),
        ...
        if (existingSubmission) {
          console.log(
            `Found existing submission: ${existingSubmission._id} in simulation ${simulationId}`
          );
          return res.json({
            success: true,
            message: "You have already submitted a solution for this problem",
    ...
    console.error(
      `Submission processing error for simulation ${simulationId}:`,
      error
    );
    res.status(500).json({
      success: false,
...
router.get("/simulations/:simulationId/leaderboard", async (req, res) => {
  try {
    ...
    console.log(
      `Fetching leaderboard for simulation ${simulationId} with limit ${limit}`
    );
...
router.get("/optimalsolution/:simulationId/:problemId", async (req, res) => {
  try {
    ...
    console.log(
      `Fetching optimal solution for problem ${problemId} in simulation ${simulationId}`
    );
