...
router.get('/simulations/:simulationId/leaderboard', async (req, res) => {
  try {
    ...
    // 3. Fetch sorted results: Highest Total Score DESC, Lowest Total Time Taken ASC
    const leaderboardEntries = await Leaderboard.find({ simulationId })
      .sort({
        totalScore: -1,      // Primary: Highest Score
        totalTimeTaken: 1,   // Secondary: Lowest Time Taken (Faster)
        lastSubmissionTime: 1 // Tertiary: Earlier Submission
      });
