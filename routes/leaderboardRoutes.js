...
async function calculateMcqScore(attemptId) {
  try {
    ...
    console.log(
      `MCQ score calculation: ${correctAnswers} correct out of ${totalQuestions}, score: ${score}`
    );
    return score;
  } catch (error) {
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      console.log(
        `Updating leaderboard for user ${userId} in simulation ${simulationId}`
      );
      console.log(
        `Received user data: username=${username}, college=${college}`
      );
      ...
      console.log(
        `Final values - username: ${usernameFinal}, college: ${collegeFinal}`
      );

      // Calculate weighted total score
      const totalScore = calculateWeightedTotalScore(mcqScore, totalDsaScore);
      console.log(
        `Weighted score calculation - MCQ (${mcqScore} × ${MCQ_WEIGHT}) + DSA (${totalDsaScore} × ${DSA_WEIGHT}) = ${totalScore}`
      );
      ...
      const lastSubmissionTime =
        dsaSubmissions.length > 0
          ? Math.max(
              ...dsaSubmissions.map((s) => new Date(s.createdAt).getTime())
            )
...
router.get("/simulations/:simulationId/leaderboard", async (req, res) => {
  try {
    ...
    const leaderboardEntries = await Leaderboard.find({ simulationId }).sort({
      totalScore: -1,
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      console.log(
        `Starting leaderboard refresh for simulation ${simulationId}`
      );
      if (currentUser) {
      ...
      console.log(
        `Found ${participantIds.length} participants for simulation ${simulationId}`
      );
      ...
      for (const userId of participantIds) {
        try {
          console.log(
            `Processing user ${userId} for simulation ${simulationId}`
          );
          ...
          if (currentUser && currentUser.userId === userId) {
            ...
            console.log(
              `Using current user data: username=${username}, college=${college}`
            );
          }
          ...
          if (!participant) {
            console.log(
              `Participant not found for user ${userId} in simulation ${simulationId}`
            );
            failureCount++;
            continue;
          }
          ...
          console.log(
            `Found participant data for user ${userId}: MCQ attempt ID: ${
              participant.mcqAttemptId
            }, DSA submissions: ${participant.dsaSubmissionIds?.length || 0}`
          );
          ...
          console.log(
            `Found ${dsaSubmissions.length} DSA submissions for user ${userId}`
          );

          // Get username from different sources
          if (!username && dsaSubmissions.length > 0) {
            // Find the newest submission
            const newestSubmission = dsaSubmissions.reduce(
              (newest, current) => {
          ...
          for (const submission of dsaSubmissions) {
            ...
            totalDsaScore += score;
            console.log(
              `DSA score for user ${userId}, problem ${submission.problemId}: ${score}`
            );
          }
          ...
          console.error(
            `Error updating leaderboard for user ${userId}:`,
            error
          );
          failureCount++;
        }
      ...
      console.log(
        `Leaderboard refresh completed for simulation ${simulationId}. Success: ${successCount}, Failed: ${failureCount}`
      );
