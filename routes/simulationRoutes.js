...
const initializeSimulations = async () => {
  try {
    ...
    if (count === 0) {
      ...
      console.log(
        `Found ${count} existing simulations. Skipping initialization.`
      );
    }
  } catch (error) {
...
router.put("/participants/:userId/:simulationId/mcq", async (req, res) => {
  try {
    ...
    console.log("Updating MCQ attempt ID:", {
      userId,
      simulationId,
      mcqAttemptId,
    });
...
router.put("/participants/:userId/:simulationId/dsa", async (req, res) => {
  try {
    ...
    console.log("Adding DSA submission ID:", {
      userId,
      simulationId,
      dsaSubmissionId,
    });
...
router.get("/:simulationId/status", async (req, res) => {
  try {
    ...
    const timeUntilAvailable = simulation.resultsAvailableTime
      ? Math.max(0, simulation.resultsAvailableTime - now)
      : 0;
...
router.put("/:simulationId/availability", async (req, res) => {
  try {
    ...
    if (resultsAvailableTime !== undefined) {
      updates.resultsAvailableTime = resultsAvailableTime
        ? new Date(resultsAvailableTime)
        : null;
    }
